...
import { AccountBillingPaymentMethod } from 'src/sections/@dashboard/user/account';
...
export default function VerifyCodeForm() {
  ...
  const onSubmit = async (data: FormValuesProps) => {
    try {
      ...
      enqueueSnackbar('Xác nhận email thành công ');
      setTimeout(() => {
        window.location.reload();
      }, 2000);
    } catch (error) {
      console.error(error);
      enqueueSnackbar('Mã xác nhận email không chính xác', { variant: 'error' });
    }
  };
